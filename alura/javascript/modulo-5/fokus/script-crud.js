...
const btnRemoverTarefasTodas = document.querySelector('#btn-remover-todas');
...
document.addEventListener('FocoAtivo', () => {
...
const removerTarefas = (apenasCompletas) => {
    const seleterTarefas = apenasCompletas ? '.app__section-task-list-item-complete' : '.app__section-task-list-item';
    ulTarefas.querySelectorAll(seleterTarefas)
        .forEach(elemento => {
            elemento.remove();
        });
...
btnRemoverTarefasTodas.onclick = () => removerTarefas(false);